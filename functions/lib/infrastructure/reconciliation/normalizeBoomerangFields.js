...
exports.normalizeBoomerangFields = void 0;
const firebase_1 = require("../firebase");
const functions = __importStar(require("firebase-functions/v1"));
const constants_1 = require("../../config/constants");
const Timestamp = firebase_1.admin.firestore.Timestamp;
...
function normalizeCreatedAt(value) {
    ...
    if (value == null)
        return null; // nothing to derive from; leave as-is
    ...
    if (typeof value === 'object') {
        ...
        if (typeof seconds === 'number') {
            const nanos = typeof (obj._nanoseconds ?? obj.nanoseconds) === 'number'
                ? (obj._nanoseconds ?? obj.nanoseconds)
                : 0;
 ...
 */
function normalizeCount(value, authoritative) {
    let current = null;
    ...
    if (current != null)
        return null; // already valid, no authoritative override
    if (typeof value === 'number' && Number.isFinite(value)) {
 ...
 */
exports.normalizeBoomerangFields = functions
    ...
    while (true) {
        ...
        for (const doc of page.docs) {
            ...
            if (nextCreatedAt)
                fix.createdAt = nextCreatedAt;
            const likedByLength = Array.isArray(data.likedBy)
                ? data.likedBy.filter((u) => typeof u === 'string' && u).length
                : undefined;
            ...
            if (nextComments != null)
                fix.commentsCount = nextComments;
...
//# sourceMappingURL=normalizeBoomerangFields.js.map